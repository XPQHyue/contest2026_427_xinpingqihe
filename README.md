...
> **队伍**：contest2026_427_xinpingqihe（XPQHyue 队长 · 程秋霞 成员） · **赛道**：③ 新硬件平台适配（主线） · ① AI 硬件产品创新为规划中、未实现
...
| [`docs/03_技术报告.md`](docs/03_技术报告.md) | 架构、实现、性能实测、8 个关键问题修复、附录 | 技术评委 |
...
- 端侧运动识别与主动交互（运动模式识别 / 主动弹建议 / 语音快捷启动）——**未实现**（i18n 标"教练规划中"）。
  ...
  "摆动 → 自动跑实验"的主动场景**已实现但默认关闭**（真机会与 GUI 抢传感器/抢切页），后续方向见同文档 §5。
...
见 `docs/06_真机验证记录.md` §6）、演示视频未录制（脚本已按真机重写）。详见 README 第八节。
...
| ③ | **PhyWear 应用** | LVGL 全中文腕上物理工坊：**16 个主页面**（原始传感器 **6 页**、力学 3、声学 2、工具 4、计时 4、设置/关于等） | **44 个手写源文件 / 13,558 行**（另有 5 个生成的 CJK 字体文件） |
...
- **线上传播**：作品过程视频已在 **Bilibili 公开发布 2 条**（对应"商业潜力"中"线上人气与
  ...
  这两条为**过程记录/传播视频**，**不等同于提交要求的正式演示视频**（正式演示视频仍需按
  脚本重新剪辑，见第八节第 5 条）。播放/点赞数据以平台页面为准，本仓库不转录。
...
│   ├── 03_技术报告.md             # 架构/实现/性能/8 个关键问题修复 + 附录
...
│   │   └── a2-proactive-20260913/ #   主动场景三段证据（现已默认关闭）
│   ├── figures/                   # 架构图 / 数据流图
...
| **DeepSeek Harness** | **2,079,471,561**（8,229 次请求） | ❌ 否（非官方支持工具） | `docs/token_usage/dsh/`（用量 CSV）+ `supplementary/dsh-logs/` |
| **合计** | **约 2,530,830,065** | — | — |
> ✅ Token 数字**完全由导出文件支撑、逐行解析、无估算**：
> MiMo 451,358,504（5,398 次请求，2026-08/09 月度记录）；DSH 2,079,471,561（8,229 次请求，2026-08-13~09-11）。
...
> 不计入 AI 工时统计（其 **2,079,471,561 tokens** 用量已如实单列）。本仓 AI Coding 日志仅含
