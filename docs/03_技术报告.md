...
- **代码结构**：`phywear_ui.c`（屏幕栈管理/宫格/列表）、`phywear_raw.c`（原始传感器 6 页横滑）、
  ...
  截至 2026-09-15：**44 个非字体源文件 / 14,911 行**，另有 5 档 CJK 字体文件（生成物）。

**UI 第二批 (a)：原始传感器三轴页「数值 + 迷你实时曲线」（2026-09-15）**

三个三轴页（加速度/陀螺/地磁）由原来的"每轴一个大数字（48 px）+ 右侧留白"改为：

| 区域 | 内容 |
|---|---|
| 页眉 0..34 | 传感器名（24 px 板块色）+ **单位**（20 px，右上角；数值列不再带单位，因为一列 126 px 放不下 `+1234.5 dps`） |
| 数值行 36..98 | **一行三列**：轴字母 X/Y/Z（20 px，固定用系列色红/绿/蓝，三页一致）+ 数值（28 px） |
| 曲线区 112..298 | 一块 **358×186** 的 `pw_scope` 缓冲，竖直切成 **3 条泳道（各 62 px）**，与上方数值列左右对齐 |
| 标注行 306..324 | 左下 **纵轴满量程**（自动量程，如 `+/-2 g`）+ 右下 **时间窗**（`8 s`） |

- **零新增 SRAM 缓冲**：曲线复用 `pw_scope`（CPU 光栅化 RGB565 → `lv_image` → EPIC 硬件 blit），
...
- **历史样本**用 `int16_t g_raw_hist[3][80]`（80 × 100 ms = 8.0 s），**三轴页共用一份**；
  ...
  这样自动量程变化不会污染历史数据的量纲。
- **自动量程**：按 **整条窗口**（3 轴 × 80 点）的峰值取 1-2-5 步进档（涨快落慢：回落需低于半量程），
  下限/上限按传感器量程约束（加速度 1..8 g、陀螺 20..2000 dps、地磁 100..30000 mG）。
- **泳道颜色**固定 X=红 / Y=绿 / Z=蓝（跨三页一致，便于对照），色板取自本队自定义系列色
  （已整体平移避开 phyphox 色板，见 `NOTICE.md`）。
- 真机截图见 `docs/evidence/ui-batch2-20260915/`（模拟器 6 张 + 真机 4 张）。
...
| 工具 | 加速度频谱、斜面倾角、磁性标尺、磁场频谱 | ✅ 真机 |
| 计时器 | 运动秒表、光学快门、声学快门 | ✅ 真机 |
| 生活 | 掌声计 | ✅ 真机 |
