...
- **代码结构**：`phywear_ui.c`（屏幕栈管理/宫格/列表）、`phywear_raw.c`（原始传感器 6 页横滑）、
  ...
  截至 2026-09-13：**44 个非字体源文件 / 13,558 行**，另有 5 档 CJK 字体文件（生成物）。
...
- `pw_graph.c` + `pw_scope.c`（1,198 行）：实时滚动曲线与可缩放/平移折线-散点图（多序列、坐标轴、Fit/Auto）。
- 典型链路（摆测 g）：IMU 角速度 50 Hz × 500 点（≈10 s）→ 自相关求周期 T → `g = 4π²L/T²`；真机实测误差约 **1.8%**（L=0.5 m）。

### 4.4 国际化与 CJK 字体子集（本队）

- **i18n**：单份 ID 枚举 + EN/ZH 双表，运行时可切换（`PW_STR()`）；当前 **228 条 ID**。
...
- **声学闭环实测**（真机）：静音环境噪声约 570 LSB；扬声器放 1 kHz 正弦时麦克风峰值 **12,000 LSB（−8.4 dBFS）**
  → 喇叭→空气→麦克风链路成立。
...
- **线程安全解耦**：LVGL 非线程安全，Agent 线程**不直接调 LVGL**；切页请求写入互斥邮箱（`pw_ai.c`），
  由 GUI 主循环轮询执行。
- **自定义 Skill（大赛硬性项）**：`phywear-physics-coach.md`（3,779 B）——
...
> 声学闭环电平（−8.4 dBFS）、启动稳定性（5/5、6/6）、Skill 摘要字节数与 md5。
...
- **附带**：麦克风增益 0 → **+30 dB**——0 dB 时静音与大声都只有 15~40 LSB，+30 dB 后环境噪声约 570 LSB、
  1 kHz 正弦下峰值 12,000 LSB（−8.4 dBFS），声学闭环成立。
...
### 6.8 ROM 只打印 `SFBL` 不启动（ftab 缺失，2026-09-12）
...
| 板块 | 内容 | 状态 |
|---|---|---|
| 原始传感器 | 加速度 / 陀螺 / 磁力 / 环境光 / **麦克风** / **扬声器**（6 页横滑，大字号实时） | ✅ 真机 |
