...
/* The agent loop is the local brain (message bus + offline intent table +
 * tool execution).  It must run even when no network is available: PhyWear's
 * proactive events and the keyword intents work fully offline.  Only the
 * network-dependent channels are gated on connectivity. */

static bool g_agent_loop_started;

static int ensure_agent_loop_started(void)
{
    if (g_agent_loop_started) {
        return OK;
    }
...
#endif
        if (ensure_agent_loop_started() != OK)
...
int ai_agent_main(int argc, char* argv[])
{
    ...
    /* Start the loop here, not only from the network watcher: on boards with
     * no network (the real Huangshan Pi) the watcher never reaches
     * "connected", which used to leave the agent running but deaf - inbound
     * messages were queued and never processed. */

    if (ensure_agent_loop_started() != OK)
        syslog(LOG_WARNING, "[%s] agent loop start failed\n", TAG);
