 ...
 * 把长度裁剪到 UTF-8 字符边界。
 *
 * 技能摘要/标题/描述都按固定字节数截断，中文一个汉字占 3 字节，直接按字节
 * 截断会把汉字切成半截，产生非法 UTF-8。该字符串会进入 system prompt 并
 * 被 cJSON 原样放进请求体，服务端解析时直接回
 * `400 {"message":"Invalid JSON in request body"}`（已实测 MiMo
 * token-plan 端点即如此）。这里回退到最后一个完整字符。
 */
...
static void extract_description(FILE *f, char *out, size_t out_size)
{
    ...
    while (fgets(line, sizeof(line), f) && off < out_size - 1) {
        ...
        off += copy;
        off = utf8_safe_len(out, off);   /* 不切断多字节字符 */
        if (copy < len)
        {
            break; /* 缓冲区已满 */
        }
...
size_t skill_loader_build_summary(char *buf, size_t size)
{
    ...
    while ((ent = readdir(dir)) != NULL && off < size - 1) {
        ...
        /* Append to summary（同样按 UTF-8 字符边界截断） */
