...
#ifdef CONFIG_SF32LB52_MIC
extern int sf32lb52_mic_register(const char *devpath);
#endif

#ifdef CONFIG_SF32LB52_SPK
extern int sf32lb52_spk_register(const char *devpath);
#endif
...
/* 传感器 I2C 总线 / LDO / INT 引脚（IMU/地磁/光三者共用 PA39/PA40 + PA30/PA31） */

#if defined(CONFIG_I2C)
#  define SF32LB52_SENSOR_I2C_BUS       1
#  define SF32LB52_SENSOR_LDO_PIN       GET_PIN_2(hwp_gpio1, 30)
#  define SF32LB52_SENSOR_INT_PIN       GET_PIN_2(hwp_gpio1, 31)
#endif

#if defined(CONFIG_I2C) && defined(CONFIG_SENSORS_MMC5603)
#  define SF32LB52_MMC5603_DEVPATH      "/dev/mag0"
#endif

#if defined(CONFIG_I2C) && defined(CONFIG_SENSORS_LTR303)
#  define SF32LB52_LTR303_DEVPATH       "/dev/light0"
#endif
...
#if defined(CONFIG_I2C)
static void sf32lb52_sensor_ldo_enable(void)
{
  /* 传感器 LDO 使能（PA30 高电平），IMU/地磁/光三颗传感器共用该电源轨。
   * 与 sf32lb52_lsm6ds3_initialize 内的使能逻辑等价，幂等可重复调用。 */

  HAL_PIN_Set(PAD_PA30, GPIO_A30, PIN_NOPULL, 1);
  sifli_gpio_config(SF32LB52_SENSOR_LDO_PIN, GPIO_OUTPUT);
  sifli_gpio_write(SF32LB52_SENSOR_LDO_PIN, true);
  usleep(10000);
}
#endif

#if defined(CONFIG_I2C) && defined(CONFIG_SENSORS_MMC5603)
static int sf32lb52_mmc5603_initialize(FAR struct i2c_master_s *i2c)
{
  int ret;

  sf32lb52_sensor_ldo_enable();

  ret = mmc5603_register(SF32LB52_MMC5603_DEVPATH, i2c, MMC5603_ADDR);
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: MMC5603 register failed on I2C%d: %d\n",
             SF32LB52_SENSOR_I2C_BUS, ret);
      return ret;
    }

  syslog(LOG_INFO, "INFO: MMC5603 registered as %s\n",
         SF32LB52_MMC5603_DEVPATH);
  return OK;
}
#endif

#if defined(CONFIG_I2C) && defined(CONFIG_SENSORS_LTR303)
static int sf32lb52_ltr303_initialize(FAR struct i2c_master_s *i2c)
{
  int ret;

  sf32lb52_sensor_ldo_enable();

  ret = ltr303_register(SF32LB52_LTR303_DEVPATH, i2c, LTR303_ADDR);
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: LTR303 register failed on I2C%d: %d\n",
             SF32LB52_SENSOR_I2C_BUS, ret);
      return ret;
    }

  syslog(LOG_INFO, "INFO: LTR303 registered as %s\n",
         SF32LB52_LTR303_DEVPATH);
  return OK;
}
#endif
...
static int lcd_async_init_thread(int argc, FAR char *argv[])
{
  ...
  if (g_pending_touch_i2c != NULL)
    {
      /* The FT6146 touch VDD shares the LCD power rail (VSYS/VCC_3V3/VADD).
       * PR #31 moved BSP_LCD_PowerUp() into lcd_hw_open(), which runs after
       * this thread, so power the rail here first — otherwise the touch
       * controller has no VDD and returns 0x00 for all registers. */
      BSP_LCD_PowerUp();
      usleep(80000);
...
#if defined(CONFIG_SENSORS_LSM6DSL) || defined(CONFIG_SENSORS_MMC5603) || \
    defined(CONFIG_SENSORS_LTR303)
...
#if defined(CONFIG_SENSORS_MMC5603)
      tmpret = sf32lb52_mmc5603_initialize(i2c1);
      if (tmpret < 0)
        {
          syslog(LOG_ERR,
                 "ERROR: sf32lb52_mmc5603_initialize failed: %d\n",
                 tmpret);
        }
#endif

#if defined(CONFIG_SENSORS_LTR303)
      tmpret = sf32lb52_ltr303_initialize(i2c1);
      if (tmpret < 0)
        {
          syslog(LOG_ERR,
                 "ERROR: sf32lb52_ltr303_initialize failed: %d\n",
                 tmpret);
        }
#endif
    }
...
#ifdef CONFIG_SF32LB52_MIC
  ret = sf32lb52_mic_register("/dev/mic0");
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: sf32lb52_mic_register failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_SF32LB52_SPK
  ret = sf32lb52_spk_register("/dev/spk0");
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: sf32lb52_spk_register failed: %d\n", ret);
    }
#endif
